<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="联保代码" prop="userNum" width="120" />
        <el-table-column align="left" label="小程序类型" prop="miniType" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.miniType,miniOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="小程序标识" prop="openid" width="120" />
        <el-table-column align="left" label="手机号" prop="phone" width="120" />
        <el-table-column align="left" label="实名状态" prop="realNameState" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.realNameState,realNameStateOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateMiniUserFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="实名状态:"  prop="realNameState" >
          <el-select v-model="formData.realNameState" placeholder="请选择" style="width:100%" :clearable="true" >
...
const getTableData = async() => {
  const table = await getMiniUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteMiniUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMiniUserFunc = async(row) => {
    const res = await findMiniUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteMiniUserFunc = async (row) => {
    const res = await deleteMiniUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

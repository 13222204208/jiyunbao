<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="门店电话">
         <el-input v-model="searchInfo.storePhone" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="门店用户" prop="uid" width="120" />
        <el-table-column align="left" label="门店名称" prop="storeName" width="120" />
        <!-- <el-table-column align="left" label="门店头像" prop="storeAvatar" width="120" /> -->
        <el-table-column align="left" label="主营品类" prop="category" width="120" />
        <el-table-column align="left" label="门店电话" prop="storePhone" width="120" />
        <el-table-column align="left" label="门店地址" prop="storeAddress" width="120" />
        <!-- <el-table-column align="left" label="经度" prop="longitude" width="120" />
        <el-table-column align="left" label="纬度" prop="latitude" width="120" /> -->
        <el-table-column align="left" label="门店公告" prop="notice" width="120" />
        <el-table-column align="left" label="结算模式" prop="close" width="120" />
        <el-table-column align="left" label="服务设置" prop="service" width="120" />
        <el-table-column align="left" label="合同协议中心" prop="agreement" width="120" />
        <el-table-column align="left" label="合作状态" prop="status" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.status,storeStateOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateAppStoreFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getAppStoreList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteAppStoreByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateAppStoreFunc = async(row) => {
    const res = await findAppStore({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteAppStoreFunc = async (row) => {
    const res = await deleteAppStore({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

<template>
  <div>
        ...
        <el-form-item label="商户号">
         <el-input v-model="searchInfo.mercId" placeholder="搜索条件" />
           ...
           <el-form-item label="支付方式" prop="payType">
            <el-select v-model="searchInfo.payType" clearable placeholder="请选择" @clear="()=>{searchInfo.payType=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="备注" prop="subject" width="120" />
        <el-table-column align="left" label="支付方式" prop="payType" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.payType,payTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="支付金额" prop="totalAmount" width="120" />
        <el-table-column align="left" label="买家ID" prop="buyer_id" width="120" />
        <el-table-column align="left" label="appid" prop="appid" width="120" />
        <el-table-column align="left" label="支付状态" prop="payState" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.payState,payStateOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateAppOrderPayFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="支付方式:"  prop="payType" >
          <el-select v-model="formData.payType" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in payTypeOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="支付状态:"  prop="payState" >
          <el-select v-model="formData.payState" placeholder="请选择" style="width:100%" :clearable="true" >
...
const getTableData = async() => {
  const table = await getAppOrderPayList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteAppOrderPayByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateAppOrderPayFunc = async(row) => {
    const res = await findAppOrderPay({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteAppOrderPayFunc = async (row) => {
    const res = await deleteAppOrderPay({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
